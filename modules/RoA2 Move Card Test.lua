...
local function calcShieldSafety(result, mode, active, custom, articleList)
	...
	if articleList ~= nil and #articleList > 0 then
		for _, article in ipairs(articleList) do
			if article.moveID == result.moveID and not article.bIsAttachedToOwner then
				local base_shield_hitpause = result["HitpauseMultiplier"] * result.Damage * result.ShieldHitpauseMultiplier
				if (article.bIsProjectile) then
					base_shield_hitpause = base_shield_hitpause * 0.5
				end
				local full_shield_hitpause = math.min(math.max(math.floor(base_shield_hitpause),3),24)

				stun = stun - 1 + full_shield_hitpause
				break
			end
		end
	end
	...
	if custom == 'N/A' then
		return stun .. " stun " .. tooltip("ⓘ", "Effective shieldstun, including shield hitpause for articles where needed")
	end
...
local function showHitUniques(result, unique, articleList)
	local listOfUniques = {}
	...
	for _, article in ipairs(articleList) do
		if article.moveID == result.moveID then
...
local function getHits(mode, result, hitData, articleList)
	...
	hitRow
		...
		:tag("td"):wikitext(calcShieldSafety(result, mode, hitData.active, hitData.shield, articleList))
	:done()
	return hitRow
end
...
local function createMode(mode, motherhits, hitresults, articleList, throwresults)
	-- Frame Window
	...
	if mode.hitID ~= nil then
		...
		if hitresults ~= nil then 
			for k, v in ipairs(hitresults) do
				hitsWindow:node(getHits(mode, hitresults[k], motherhits[hitresults[k].moveID .. hitresults[k].nameID], articleList))
				local uniqueRow = showHitUniques(hitresults[k], motherhits[hitresults[k].moveID .. hitresults[k].nameID].unique, articleList)
				if uniqueRow then
...
local function createAdvMode(mode, motherhits, hitresults, articleList, throwresults)
	...
	finalreturn:wikitext("''For full knockdown and crouch cancel percents, please visit the character's matchup page for exact numbers.''")
...
local function getCardHTML(chara, attack, desc, advDesc)
	...
	for i in pairs(tableData) do
		...
		if mode.hitID ~= nil then
			...
			whereField = whereField .. table.concat(whereList, " or ") .. ")"local args = { where = whereField, orderBy = "_ID" }
		...
		queues[mode] = {mode = mode, hits = hits, hit_results = hit_results, throw_results = throw_results, article_results = articleList}
	end
	if #tableData > 1 then
		local object = {}
		local advObject = {}
		for i in pairs(tableData) do
			...
			object["content" .. i] = createMode(queues[tableData[i]].mode, queues[tableData[i]].hits, queues[tableData[i]].hit_results, queues[tableData[i]].article_results, queues[tableData[i]].throw_results)
			advObject["label" .. i] = tableData[i].mode
			advObject["content" .. i] = createAdvMode(queues[tableData[i]].mode, queues[tableData[i]].hits, queues[tableData[i]].hit_results, queues[tableData[i]].article_results, queues[tableData[i]].throw_results)
		end
		...
		if(queues[tableData[1]]) then
			local i = 1
			paletteSwap:node(createMode(queues[tableData[i]].mode, queues[tableData[i]].hits, queues[tableData[i]].hit_results, queues[tableData[i]].article_results, queues[tableData[i]].throw_results)):done()
			nerdSection:node(createAdvMode(queues[tableData[i]].mode, queues[tableData[i]].hits, queues[tableData[i]].hit_results, queues[tableData[i]].article_results, queues[tableData[i]].throw_results)):done()
		end
	end
