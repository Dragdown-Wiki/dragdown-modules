...
local function makeAngleDisplay(angle, flipper)
	...
	if angle < 360 then
		div1:tag("div")
			...
			:wikitext("[[File:RushRev_AngleComplex_BG.svg|256px|link=]]")
			:done()
			:tag("div")
			:css("z-index", "1")
			...
			:css("top", "0")
			:css("left", "0")
			:wikitext("[[File:RushRev_AngleComplex_MG.svg|256px|link=]]")
			:done()
			:tag("div")
			:css("transform", "rotate(-" .. angle .. "deg)")
			:css("z-index", "2")
			...
			:css("left", "0")
			:css("transform-origin", "center center")
			:wikitext("[[File:RushRev_AngleComplex_FG.svg|256px|link=]]")
			:done()
		div1:wikitext("Angle Flipper: " .. flipper)
		div1:done()
		display:node(div1):wikitext("[[File:RushRev_AngleComplex_Key.svg|256px|link=]]")
		display:done()
	else
		div1:wikitext("Angle Flipper: " .. flipper)
		div1:done()
...
local function getImageGallery(chara, attack)
	local tables = "RushRev_MoveMode, RushRev_MoveMode__image, RushRev_MoveMode__caption"
	local fields = "image, caption"
	local args = {
		join = "RushRev_MoveMode__image._rowID=RushRev_MoveMode._ID, RushRev_MoveMode__image._rowID=RushRev_MoveMode__caption._rowID, RushRev_MoveMode__image._position=RushRev_MoveMode__caption._position",
		where = 'RushRev_MoveMode.chara="' .. chara .. '" and RushRev_MoveMode.attack="' .. attack .. '"',
		orderBy = "_ID",
		groupBy = "RushRev_MoveMode__image._value",
	}
	local results = cargo.query(tables, fields, args)
	...
	if acquiredImages ~= '<div class="attack-gallery-image"></div>' then
		...
		container:wikitext(
			table.concat(
				getImagesWikitext({
					{
						file = "RushRev_" .. chara .. "_" .. attack .. "_0.png",
